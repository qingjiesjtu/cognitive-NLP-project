/*------------------------------------------------------------------*/
/* Adapted from the work of Telecom Paristech - J-L. Dessalles 2018 */
/* about Symbolic Natural Language Processing                       */
/* Add the part to set necessity dynamically                        */
/*------------------------------------------------------------------*/
...
%:- consult('rel_util.pl').	% chargement d'utilitaires si le systeme graphique est present
...
% qsave_program('paradise',[goal=go,autoload=true, stand_alone=true]).
	...
	go :-
		...
		set_preference, % to set necessities on the fly
	...
	% set necessities on the fly
	set_preference :-
		talk(['How is your preference for attend_exam? Enter [certainly, highly_likely, probably, 
		even, probably_not, unlikely, chances_are_slight](ended by ''.'').']),
		read(W1),
		words2necessity(W1,N1),
		assert(preference(attend_exam,N1)),
		talk(['How is your preference for slip? Enter [certainly, highly_likely, probably, 
		even, probably_not, unlikely, chances_are_slight](ended by ''.'').']),
		read(W2),
		words2necessity(W2,N2),
		assert(preference(slip,N2)),
		talk(['How is your preference for wet_feet? Enter [certainly, highly_likely, probably, 
		even, probably_not, unlikely, chances_are_slight](ended by ''.'').']),
		read(W3),
		words2necessity(W3,N3),
		assert(preference(wet_feet,N3)),
		talk(['How is your preference for expensive? Enter [certainly, highly_likely, probably, 
		even, probably_not, unlikely, chances_are_slight](ended by ''.'').']),
		read(W4),
		words2necessity(W4,N4),
		assert(preference(expensive,N4)).

	% table for probability words and necessities
	words2necessity('certainly', 30).
	words2necessity('highly_likely', 20).
	words2necessity('probably', 10).
	words2necessity('even', 0).
	words2necessity('probably_not', -10).
	words2necessity('unlikely',-20).
	words2necessity('chances_are_slight', -30).
